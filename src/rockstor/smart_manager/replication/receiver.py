...
class Receiver(ReplicationMixin, Process):
    ...
    def __init__(self, identity, meta):
        ...
        self.snap_dir = ('%s%s/.snapshots/%s' % (settings.MNT_PT,
                                                 self.dest_pool, self.sname))
    ...
    def _sys_exit(self, code):
        if (self.rp is not None and self.rp.returncode is None):
            try:
                ...
                logger.error('Id: %s. Exception while terminating '
                             'the btrfs-recv process: %s'
                             % (self.identity, e.__str__()))
        self.ctx.destroy(linger=0)
        if (code == 0):
            logger.debug('Id: %s. meta: %s Receive successful'
        ...
        try:
            ...
            if (self.rtid is not None):
                try:
                    ...
                    msg = ('Id: %s. Exception while updating receive '
                           'trail for rtid(%d).'
            ...
            if (self.ack is True):
                try:
                    ...
                    self.dealer.send_multipart(
                        ['receiver-error', b'%s. Exception: %s' %
                    ...
                    if (socks.get(self.dealer) == zmq.POLLIN):
                        ...
                        logger.debug('Id: %s. Response from the broker: %s'
                                     % (self.identity, msg))
                    else:
                        logger.debug('Id: %s. No response received from '
                                     'the broker' % self.identity)
                except Exception as e:
                    msg = ('Id: %s. Exception while sending %s back '
                           'to the broker. Aborting'
    ...
    def _delete_old_snaps(self, share_name, share_path, num_retain):
        ...
        if (oldest_snap is not None):
            if (self.delete_snapshot(share_name, oldest_snap)):
                return self._delete_old_snaps(share_name, share_path,
                                              num_retain)
    ...
    def run(self):
        ...
        with self._clean_exit_handler():
            ...
            term_commands = ('btrfs-send-init-error',
