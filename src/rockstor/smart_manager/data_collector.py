...
from system.pinmanager import (has_pincard, username_to_uid,  # noqa E402
                               email_notification_enabled, reset_random_pins,
                               generate_otp)
...
class LogManagerNamespace(RockstorIO):
    ...
    def on_connect(self, sid, environ):
        ...
        self.emit('logwelcome',
                  {
                      'key': 'logManager:logwelcome',
                      'data': 'Welcome to Rockstor LogManager'
                  })
    ...
    def build_log_path(self, selectedlog):
        ...
        return '{0}{1}'.format(self.logs[selectedlog]['logdir'],
                               self.logs[selectedlog]['logfile'])
    ...
    def find_rotating_logs(self):
        ...
        for log_key in log_keys:
            ...
            if ('excluded' in self.logs[log_key]):
                rotated_logs = list(set(rotated_logs)-{l for e in self.logs[log_key]['excluded'] for l in rotated_logs if e in l})  # noqa E501
            ...
            for current_rotated in rotated_logs:
                ...
                rotated_logs_list.append({'log': rotated_key, 'logfamily':
                                          log_key})

        self.emit('rotatedlogs',
                  {
    ...
    def on_readlog(self, sid, reader, logfile):
        ...
        def live_reader(log_path):
            ...
            self.livereader_process = Popen(read_command, bufsize=1,
                                            stdout=PIPE)
            ...
            for disk in cur_stats.keys():
                if (disk in prev_stats):
                    ...
                    disks_stats.append({
                        ...
                        'ts': str(datetime.utcnow().replace(tzinfo=utc).isoformat())  # noqa E501
                        })

            self.emit('top_disks',
                      {
                          'key': 'diskWidget:top_disks',
                          'data': disks_stats
                      })
            return cur_stats
...
class MemoryWidgetNamespace(RockstorIO):
    ...
    def send_meminfo_data(self):
        ...
        while self.switch:
            ...
            (total, free, buffers, cached, swap_total, swap_free, active,
             inactive, dirty,) = (None,) * 9
    ...
    def send_service_statuses(self):
        ...
        while self.start:
            ...
            for service in Service.objects.all():
                ...
                data[service.name] = {}
                output, error, return_code = service_status(service.name,
                                                            config=config)
