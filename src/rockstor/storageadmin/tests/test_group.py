...
class GroupTests(APITestMixin):
    # Proposed fixture "test_group.json" was "fix3.json"
    ...
    def test_post_requests(self):
        ...
        self.assertEqual(
            response.status_code, status.HTTP_400_BAD_REQUEST, msg=response.content
        )
        self.assertEqual(
            response.data[0], "Group (root) already exists. " "Choose a different one."
        )
        ...
        # invalid gid
        data = {"groupname": "ngroup2", "gid": 100}
        response = self.client.post(self.BASE_URL, data=data)
        self.assertEqual(
            response.status_code, status.HTTP_400_BAD_REQUEST, msg=response.data
        )
