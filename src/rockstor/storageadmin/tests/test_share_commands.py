...
class ShareCommandTests(APITestMixin):
    # Proposed fixture "test_share_commands.json" was "fix1.json"
    ...
    def setUpClass(cls):
        ...
        cls.patch_create_repclone = patch(
            "storageadmin.views.share_command." "create_repclone"
        )
        cls.mock_create_repclone = cls.patch_create_repclone.start()
        cls.mock_create_repclone.return_value = Response('{"message": "ok!"}')

        cls.patch_create_clone = patch(
            "storageadmin.views.share_command." "create_clone"
        )
    ...
    def test_clone_command(self, mock_share):
        ...
        r = self.client.post("{}/{}/clone".format(self.BASE_URL, shareId), data=data)
        self.assertEqual(
    ...
    @mock.patch("storageadmin.views.share_command.Share")
    def test_rollback_command(self, mock_share, mock_snapshot, mock_nfs, mock_samba):
        """
        ...
        mock_share.objects.get.side_effect = Share.DoesNotExist
        r = self.client.post("{}/{}/rollback".format(self.BASE_URL, shareId), data=data)
        self.assertEqual(
            r.status_code, status.HTTP_500_INTERNAL_SERVER_ERROR, msg=r.data
        )
        ...
        e_msg = (
            "Share (rshare2) cannot be rolled back as it is exported via "
        ...
        e_msg = (
            "Share (rshare2) cannot be rolled back as it is shared "
