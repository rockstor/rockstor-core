...
class AppliancesTests(APITestMixin, APITestCase):
    ...
    def test_post_requests(self):
        ...
        e_msg = ('Failed to authenticate on remote appliance. Verify port number, id and secret are correct and try again.')
        ...
        e_msg = ('Failed to get remote appliance information. Verify all inputs and try again.')
        self.assertEqual(response.data['detail'], e_msg)
        self.mock_api_call.side_effect = None
        
        # ip already exists
        data = {'ip': '192.168.56.101', 'mgmt_port': '443', 'client_id':'', 'client_secret':'', 'current_appliance': False }
        response = self.client.post(self.BASE_URL, data=data)
        self.assertEqual(response.status_code, 
                         status.HTTP_500_INTERNAL_SERVER_ERROR, msg=response.data)
        
        e_msg = ('The appliance with ip = 192.168.56.101 already exists and cannot be added again')
        self.assertEqual(response.data['detail'], e_msg)
        
        # invalid management port
        data = {'ip': '1.1.1.1', 'mgmt_port': 'invalid', 'client_id':'', 'client_secret':'', 'current_appliance': False }
        response = self.client.post(self.BASE_URL, data=data)
        self.assertEqual(response.status_code, 
                         status.HTTP_500_INTERNAL_SERVER_ERROR, msg=response.data)
