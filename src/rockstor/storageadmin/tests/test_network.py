...
class NetworkTests(APITestMixin, APITestCase):
    ...
    def setUpClass(cls):
        ...
        cls.patch_config_network_device = patch('storageadmin.views.network.config_network_device')
    ...
    def test_put(self):
        ...
        # netmask set to None
        data = {'method':'manual', 'ipaddr':'192.168.56.101', 'netmask':None, 'gateway':'', 'dns_servers':'', 'itype':'io'}
        response = self.client.put('%s/enp0s3' % self.BASE_URL, data=data)
        self.assertEqual(response.status_code, 
                         status.HTTP_500_INTERNAL_SERVER_ERROR, msg=response.data)
        e_msg = 'Provided netmask value(None) is invalid. You can provide it in a IP address format(eg: 255.255.255.0) or number of bits(eg: 24)'
        self.assertEqual(response.data['detail'], e_msg)
        
        # Invalid netmask
        data = {'method':'manual', 'ipaddr':'192.168.56.101', 'netmask':'111', 'gateway':'', 'dns_servers':'', 'itype':'io'}
        response = self.client.put('%s/enp0s3' % self.BASE_URL, data=data)
        self.assertEqual(response.status_code, 
                         status.HTTP_500_INTERNAL_SERVER_ERROR, msg=response.data)
        e_msg = 'Provided netmask value(111) is invalid. Number of bits in netmask must be between 1-32'
        self.assertEqual(response.data['detail'], e_msg)
        
        # happy path
        data = {'method':'manual', 'ipaddr':'192.168.56.101', 'netmask':'225.225.225.0','gateway':'','dns_servers':'', 'itype':'io'}
        response = self.client.put('%s/enp0s3' % self.BASE_URL, data=data)
        self.assertEqual(response.status_code, 
                         status.HTTP_200_OK, msg=response.data)
        ...
        # Setting network interface itype to management when the othet network is already set to management
        data = {'method':'auto', 'itype':'management'}
        response = self.client.put('%s/enp0s8' % self.BASE_URL, data=data)
        self.assertEqual(response.status_code, 
                         status.HTTP_500_INTERNAL_SERVER_ERROR, msg=response.data)
        e_msg = 'Another interface(enp0s3) is already configured for management. You must disable it first before making this change.'                 
        self.assertEqual(response.data['detail'], e_msg)
        
        # provide ipaddress thats already been used by another interface
        data = {'method':'manual', 'ipaddr':'10.0.3.15', 'netmask':'225.225.225.0','gateway':'','dns_servers':'', 'itype':'io'}
        response = self.client.put('%s/enp0s3' % self.BASE_URL, data=data)
        self.assertEqual(response.status_code, 
                         status.HTTP_500_INTERNAL_SERVER_ERROR, msg=response.data)
        e_msg = 'IP: 192.168.56.101 already in use by another interface: enp0s8'
        self.assertEqual(response.data['detail'], e_msg)
