...
"""
Using fixture fix5.json where hard coded data to pre-populate database before the tests run.
Created pool1 
Created share1, share2 using pool1 
Created snapshot snap1 for share1 with uvisible false
Created snapshot snap2 for share2 with uvisible True
"""
class SnapshotTests(APITestMixin, APITestCase):
    ...
    def setUpClass(cls):
        ...
        cls.patch_share_usage = patch('storageadmin.views.snapshot.share_usage')
    ...
    def test_post_requests(self, mock_nfs):
        ...
        """
        # Invalid share name
        data = {'snapshot-name': 'snap3', 'shares':'invalid', 'writable':'rw', 'uvisible': 'invalid' }
        snap_name = 'snap3'
        share_name = 'invalid'
        response = self.client.post('%s/%s/snapshots/%s' % (self.BASE_URL, share_name, snap_name), data=data, sname=share_name, snap_name=snap_name)
        ...
        data = {'snapshot-name': 'snap3', 'shares':'share1', 'writable':'rw', 'uvisible': 'invalid' }
        snap_name = 'snap3'
        share_name = 'share1'
        response = self.client.post('%s/%s/snapshots/%s' % (self.BASE_URL, share_name, snap_name), data=data, sname=share_name, snap_name=snap_name)
        self.assertEqual(response.status_code,
        ...
        data = {'snapshot-name': 'snap2', 'shares':'share2', 'writable':'rw', 'uvisible': True }
        snap_name = 'snap2'
        share_name = 'share2'
        response = self.client.post('%s/%s/snapshots/%s' % (self.BASE_URL, share_name, snap_name), data=data, sname=share_name, snap_name=snap_name)
        self.assertEqual(response.status_code,
        ...
        data = {'snapshot-name': 'snap3', 'shares':'share1', 'writable':'rw', 'uvisible': True }
        snap_name = 'snap3'
        share = 'share1'
        mock_nfs.objects.filter(share=share).exists.return_value = True
        response = self.client.post('%s/%s/snapshots/%s' % (self.BASE_URL, share, snap_name), data=data, sname=share, snap_name=snap_name)
        self.assertEqual(response.status_code,
    ...
    def test_clone_command(self):
        ...
        share = 'share2'
        response = self.client.post('%s/%s/snapshots/%s' % (self.BASE_URL, share, snap_name), data=data, sname=share, snap_name=snap_name, command='clone')
        self.assertEqual(response.status_code,
                         status.HTTP_200_OK, msg=response.data)
    ...
    def test_delete_requests(self):
        ...
        response = self.client.delete('%s/%s/snapshots/%s' % (self.BASE_URL, share_name, snap_name))
        self.assertEqual(response.status_code,
                         status.HTTP_500_INTERNAL_SERVER_ERROR, msg=response.data)
        ...
        share_name = 'share1'        
        response = self.client.delete('%s/%s/snapshots' % (self.BASE_URL, share_name))
        self.assertEqual(response.status_code,
                         status.HTTP_200_OK, msg=response.data)
