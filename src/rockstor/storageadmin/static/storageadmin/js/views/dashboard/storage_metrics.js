...
StorageMetricsWidget = RockStorWidgetView.extend({
  ...
  setGraphDimensions: function() {
    ...
    this.svg = d3.select(this.el).select('#ph-storage-metrics-graph')
    .append('svg')
  ...
  renderDisks: function() {
    var _this = this;
    var diskRow = this.svg.append('g').attr('class', 'disk-row');
    
    var diskRects = diskRow.selectAll('g.disk')
    .data(_this.disks)
    .enter()
    .append('rect')
    .attr('x', function(d, i) { 
      console.log(i);
      return (_this.barWidth / _this.disks.length)*i; 
    })
    .attr('y', 0)
    .attr('width', _this.barWidth/_this.disks.length)
    .attr('height', _this.barHeight - 2);
