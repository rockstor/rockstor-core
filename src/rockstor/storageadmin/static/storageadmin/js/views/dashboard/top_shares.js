...
TopSharesWidget = RockStorWidgetView.extend({
  ...
  setData: function() {
    ...
    this.data.map(function(d) {
      d.set({'pUsed': ((d.get('rusage')/d.get('size'))*100)});
    });
  },
  ...
  renderTopShares: function() {
    ...
    .html(function(d) {
      return '<strong>' + d.get('name') + '</strong><br>' +
        '   <strong>Used:</strong> ' +
        humanize.filesize(d.get('rusage')*1024) + '<br>' +
        ' <strong>Free:</strong> ' +
        humanize.filesize((d.get('size')-d.get('rusage'))*1024);
    })
    ...
    .text(function(d) {
      ...
      return n + ' (' + humanize.filesize(d.get('rusage')*1024) +
